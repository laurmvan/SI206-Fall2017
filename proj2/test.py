...
#Use back slashes in between the front slashes so that regular expressions doesn't get confused. Make sure to return at least 2 characters after the period.
def find_urls(s):
    return_string = re.findall("https*:\/\/\S*\.\S*\w{2,}",s)
...
#search for the most read class and loop through it. Add title.text to the list to get the actual titles and then return the list.
...
#Go to the view-content class becuase it is a unique class that includes all poeple in the directory.  
...
def get_umsi_data():
    ...
    for i in range(13):
        ...
        r= requests.get(url, headers={'User-Agent': 'SI_CLASS'}) #permission to enter site
    ...
    #since I made two separate lists and I wanted to create a dicitonary using these values, I used zip to combine the names with their corresponding titles
    all_titles= dict(diction)
    return (all_titles)
    #Your code here
